...
export function pushNotification(notification) {
  return async (dispatch, getState) => {
    try {
      ...
      const timeout = compose(
        propOr(DEFAULT_TIMEOUT, notification.type),
        getNotificationsTimeout,
        getState
      )();
      ...
      for (let { id, isOpen } of currentQueue) {
        if (!isOpen) {
          ...
          const resultQueue = currentQueue.map((item) =>
            item.id === id ? newNotification : item
          );
...
export function removeNotification(id) {
  return async (dispatch, getState) => {
    ...
    let resultQueue = currentQueue.map((item) =>
